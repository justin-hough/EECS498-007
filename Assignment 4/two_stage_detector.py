    ...
    def predict_proposals(
        ...
        for _batch_idx in range(batch_size):
            ...
            for level_name in anchors_per_fpn_level.keys():
                ...
                # Step 1. Transform anchors to proposals using the function we wrote earlier, then clamp
                proposals = rcnn_apply_deltas_to_anchors(level_boxreg_deltas, level_anchors)
                proposals[:, [0, 2]] = torch.clamp(proposals[:, [0, 2]], min=0, max=image_size[1])
                proposals[:, [1, 3]] = torch.clamp(proposals[:, [1, 3]], min=0, max=image_size[0])
                
                # Step 2. Sort all proposals by their objectness, keeping self.pre_nms_topk
                if len(proposals) < self.pre_nms_topk:
                ...
                # Step 3. Apply nms and retain self.post_nms_topk
                keep = torchvision.ops.nms(proposals, logits, self.nms_thresh)
                proposals = proposals[keep]
                scores = logits[keep].reshape(-1)           

                if len(scores) >= self.post_nms_topk:
                    post_nms_scores, post_nms_indices = torch.topk(scores, self.post_nms_topk)
                    proposals_per_fpn_level_per_image[level_name] = proposals[post_nms_indices]
