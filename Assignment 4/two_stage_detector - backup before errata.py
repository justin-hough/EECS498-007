...
class RPNPredictionNetwork(nn.Module):
    ...
    def __init__(
        ...
        for layer_num in range(len(stem_channels)):
            ...
            # Define convolutions
            conv = nn.Conv2d(c_in, stem_channels[layer_num], 3, 1, 1)
        ...
        self.pred_obj = nn.Conv2d(stem_channels[-1], self.num_anchors, 1)  # Objectness conv
        self.pred_box = nn.Conv2d(stem_channels[-1], 4*self.num_anchors, 1)  # Box regression conv
    ...
    def forward(self, feats_per_fpn_level: TensorDict) -> List[TensorDict]:
        ...
        for fpn_level, fpn_features in feats_per_fpn_level.items():
            ...
            obj_logit = self.pred_obj(self.stem_rpn(fpn_features))
            obj_logit = obj_logit.reshape(batch_size, H*W*self.num_anchors)

            object_logits.update({
                fpn_level: obj_logit
                })
            
            # Predict the box regression logits
            br_delta = self.pred_box(self.stem_rpn(fpn_features))
            br_delta = br_delta.reshape(batch_size, -1, H*W*self.num_anchors).permute(0, 2, 1)
...
def generate_fpn_anchors(
    ...
    for level_name, locations in locations_per_fpn_level.items():
        ...
        for aspect_ratio in aspect_ratios:
            ...
            # Using the given formulae for area, w, and h
            area = (stride_scale * level_stride)**2
            w = math.sqrt(area / aspect_ratio)
            h = area / w
            xc, yc = locations.unsqueeze(dim=2).unbind(dim=1)
            ab = torch.hstack((xc-w/2, yc-h/2, xc+w/2, yc+h/2))
            anchor_boxes.append(ab)
            ##################################################################
            #                           END OF YOUR CODE                     #
            ##################################################################
...
def iou(boxes1: torch.Tensor, boxes2: torch.Tensor) -> torch.Tensor:
    ...
    # Get box coordinates
    x1_1, y1_1, x2_1, y2_1 = boxes1.unsqueeze(dim=2).unbind(dim=1)
    ...
    areas_i = torch.clamp((x2_i - x1_i), min=0) * torch.clamp((y2_i - y1_i), min=0)

    # Get union areas (rely on broadcasting to handle different shapes)
    areas_u = areas1 + areas2 - areas_i
...
def rcnn_get_deltas_from_anchors(
    ...
    # Go from XYXY to XYWH
    anchor_x = (anchors[:, 2:3] + anchors[:, 0:1])/2
    anchor_y = (anchors[:, 3:4] + anchors[:, 1:2])/2
    anchor_w = (anchors[:, 2:3] - anchors[:, 0:1])
    anchor_h = (anchors[:, 3:4] - anchors[:, 1:2])

    gt_x = (gt_boxes[:, 2:3] + gt_boxes[:, 0:1])/2
    gt_y = (gt_boxes[:, 3:4] + gt_boxes[:, 1:2])/2
    gt_w = (gt_boxes[:, 2:3] - gt_boxes[:, 0:1])
    gt_h = (gt_boxes[:, 3:4] - gt_boxes[:, 1:2])

    # Calculate deltas according to formula on Lecture 13, Slide 71
    deltas = torch.hstack((
        (gt_x-anchor_x)/anchor_w, # t_x
        (gt_y-anchor_y)/anchor_h, # t_y
        torch.log(gt_w/anchor_w), # t_w
        torch.log(gt_h/anchor_h) # t_h
        ))

    # Set deltas to (-1e8, -1e8, -1e8, -1e8) for "background/neutral" class
    deltas[gt_boxes[:,-1] <= -1, :] = -1e8
...
def rcnn_apply_deltas_to_anchors(
    ...
    # Get output boxes XYWH
    output_boxes_x = anchor_x + anchor_w*deltas[:, 0:1]
    output_boxes_y = anchor_y + anchor_h*deltas[:, 1:2]
    output_boxes_w = anchor_w*torch.exp(deltas[:, 2:3])
    output_boxes_h = anchor_h*torch.exp(deltas[:, 3:4])

    # Using the formula given on Lecture 13, Slide 68
    output_boxes = torch.hstack((
        output_boxes_x - output_boxes_w/2,
        output_boxes_y - output_boxes_h/2,
        output_boxes_x + output_boxes_w/2,
        output_boxes_y + output_boxes_h/2,
    ))
...
class RPN(nn.Module):
    ...
    def forward(
        ...
        locations_per_fpn_level = get_fpn_location_coords(
            ...
            device=next(iter(feats_per_fpn_level.values())).device
        )
        ...
        if self.training:
            ...
            # Step 1
            # Confusingly, batch_size_per_image is the number of anchors to sample per image in the training batch 
            num_samples = int(self.batch_size_per_image * num_images)
            fg_fraction = 0.5 # 50% foreground fraction
            fg_idx, bg_idx = sample_rpn_training(matched_gt_boxes, num_samples, fg_fraction)
            subset_idx = torch.cat((fg_idx, bg_idx))
            subset_anchor_boxes = anchor_boxes[subset_idx]
            subset_matched_gt_boxes = matched_gt_boxes[subset_idx]

            # Step 2
            gt_deltas = rcnn_get_deltas_from_anchors(subset_anchor_boxes, subset_matched_gt_boxes)

            # Step 3
            gt_objectness = gt_deltas.new_zeros(subset_idx.shape[0]) # Initialise the ground truth objectness tensor
            gt_objectness[:fg_idx.shape[0]] = 1 # The elements at the start of the tensor that correspond to the foreground indices
            loss_obj = F.binary_cross_entropy_with_logits(pred_obj_logits[subset_idx], gt_objectness, reduction='none')

            loss_box = F.l1_loss(pred_boxreg_deltas[subset_idx], gt_deltas, reduction='none')
    ...
    def predict_proposals(
        ...
        for level_name in anchors_per_fpn_level.keys():
            ...
            for _batch_idx in range(level_obj_logits.shape[0]):
                ...
                # Transform anchors to proposals using the function we wrote earlier, then clamp
                proposals = rcnn_apply_deltas_to_anchors(level_boxreg_deltas[_batch_idx], level_anchors)
                proposals[:, [0, 2]] = torch.clamp(proposals[:, [0, 2]], min=0, max=image_size[0])
                proposals[:, [1, 3]] = torch.clamp(proposals[:, [1, 3]], min=0, max=image_size[1])

                # Sort all proposals by their objectness, keeping self.pre_nms_topk
                scores, indices = torch.sort(level_obj_logits[0], descending=True)
                if len(indices) >= self.pre_nms_topk:
                    proposals = torch.index_select(proposals, 0, indices)[:self.pre_nms_topk, :]
                    scores = scores[:self.pre_nms_topk]
                else:
                    proposals = torch.index_select(proposals, 0, indices)

                # Apply nms and retain self.post_nms_topk
                keep = torchvision.ops.nms(proposals, scores, iou_threshold=self.nms_thresh)
                if len(keep) >= self.post_nms_topk:
                    proposals = proposals[keep[:self.post_nms_topk], :]
                else:
...
class FasterRCNN(nn.Module):
    ...
    def __init__(
        ...
        # Set number of in_channels to the number of out_channels from the backbone
        ...
        # Add nn.Flatten module prior to making classification prediction
        cls_pred.append(nn.Flatten())
        
        # Use linear layer for classification prediction
        linear_in_channels = roi_size[0] * roi_size[1] * stem_channels[-1]
        cls_pred.append(nn.Linear(
    ...
    def forward(
        ...
        for _idx in range(len(gt_boxes)):
            ...
            matched_gt_boxes_per_image = rcnn_match_anchors_to_gt(
                proposals_per_image,
                gt_boxes_per_image,
                iou_thresholds=(0.5, 0.5) # Results in only background and foreground matches
            )
        ...
        # Step 1
        num_samples = int(self.batch_size_per_image * num_images)
        fg_fraction = 0.25 # 25% foreground fraction
        fg_idx, bg_idx = sample_rpn_training(matched_gt_boxes, num_samples, fg_fraction)
        subset_idx = torch.cat((fg_idx, bg_idx))

        # Step 2
        subset_matched_gt_cls = matched_gt_boxes[subset_idx,-1].to(torch.long)
        subset_pred_cls_logits = pred_cls_logits[subset_idx, :].to(torch.float)

        # Step 3
        subset_matched_gt_cls += 1
        loss_cls = F.cross_entropy(subset_pred_cls_logits, subset_matched_gt_cls)
    ...
    def inference(
        ...
        # Step 1
        pred_cls_scores = F.softmax(pred_cls_logits, dim=1)[:, 1:] # Shift the predicted IDs such that the model outputs ID (0-19) for 20 VOC classes
        ...
        pred_boxes = torch.max(
            torch.min(
                pred_boxes,
                torch.tensor([W, H, W, H],
                device=pred_boxes.device)),
            torch.zeros(4, device=pred_boxes.device))
