...
class Linear(object):
    ...
    def forward(x, w, b):
        ...
        x_reshape = x.view(x.shape[0], -1) # (N, D)
        out = x_reshape.mm(w) + b # (N, D) * (D, M) + (M,) = (N, M)
        ######################################################################
        #                        END OF YOUR CODE                            #
        ######################################################################
        cache = (x, w, b)
    ...
    def backward(dout, cache):
        ...
        # dx = dout * dy/dx
        dout_dx = w # (D, M)
        dx = dout.mm(dout_dx.T) # (N, M) * (D, M) = (N, D)
        dx = dx.view(x.shape) # (N, d_1, ..., d_k)

        # -- dw --
        # dL/dw = dL/dy * dy/dw
        # dL/dw = dout * dy/dw
        # dw = dout * dy/dw
        dout_dw = x.view(x.shape[0], -1) # (N, D)
        dw = dout_dw.T.mm(dout) # (D, N) * (N, M) = (D, M)

        # -- db --
        # dL/db = dL/dy * dy/db
        # dL/db = dout * dy/db
        # db = dout * dy/db
        dout_db = torch.ones(x.shape[0], device=x.device, dtype=x.dtype) # (N,)
        db = torch.matmul(dout_db, dout) # (N,) * (N, M) = (M,)
...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        # Forward pass throught the two layer network
        ...
        dx, dw, db = Linear.backward(dx, cache2)
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        for i, (D1, D2) in enumerate(zip([input_dim, *hidden_dims], [*hidden_dims, num_classes])):
          W = torch.randn(D1, D2, dtype=dtype, device=device) * weight_scale
          b = torch.zeros(D2, dtype=dtype, device=device)
    ...
    def loss(self, X, y=None):
        ...
        for layer_num in range(1, self.num_layers):
          out, cache = Linear_ReLU.forward(out, self.params[f"W{layer_num}"], self.params[f"b{layer_num}"])
          caches.append(cache)
          if self.use_dropout:
            out, cache = Dropout.forward(out, dropout_param=self.dropout_param)
            caches.append(cache)
        ...
        for layer_num in range(self.num_layers-1, 0, -1):
          loss += 0.5 * self.reg * torch.sum(self.params[f"W{layer_num}"] * self.params[f"W{layer_num}"]) # Add L2 regularisation
          if self.use_dropout:
            dx = Dropout.backward(dx, caches.pop(-1))
          dx, dw, db = Linear_ReLU.backward(dx, caches.pop(-1))
          dw += self.reg*self.params[f"W{layer_num}"] # Add L2 regularisation
          grads.update({
              f"W{layer_num}": dw,
              f"b{layer_num}": db 
          })
...
class Dropout(object):
    ...
    def forward(x, dropout_param):
        ...
        if mode == 'train':
            ...
            out = x # Inverted dropout does not modify the test time behaviour
            ##############################################################
            #                      END OF YOUR CODE                      #
            ##############################################################
