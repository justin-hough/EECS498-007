...
class Conv(object):
    ...
    def forward(x, w, b, conv_param):
        ...
        out = x.new_zeros(N, F, Hout, Wout)
        for n in range(N):
          for f in range(F):
            for i in range(Hout):
              for j in range(Wout):
                  img = x[n, :, i*stride:i*stride+HH, j*stride:j*stride+WW]
                  out[n, f, i, j] = torch.sum(img * w[f, :, :, :]) + b[f]
    ...
    def backward(dout, cache):
        ...
        # Get the dimensions
        N, C, Hdout, Wdout = dout.shape
        F, _, HH, WW = w.shape

        # Backprop the convolution
        for n in range(N):
          for f in range(F):
            for i in range(Hdout):
              for j in range(Wdout):
        ...
        # Unpad the gradients of the input
...
class MaxPool(object):
    ...
    def forward(x, pool_param):
        ...
        out = x.new_zeros(N, C, Hout, Wout)
        for i in range(Hout):
          for j in range(Wout):
              # Get the image volume that we're finding the maximum values within
              img = x[:, :, i*stride:i*stride+pool_height, j*stride:j*stride+pool_width]
              out[:, :, i, j] = torch.amax(img, dim=(2, 3)) # Allows taking max in multiple dimensions (i.e. keep N and C, find 2D spatial max only)
    ...
    def backward(dout, cache):
        ...
        # Backprop the max pool
        for n in range(N):
            for i in range(Hdout):
              for j in range(Wdout):
                # Get the image volume that we found the maximum values within
                img = x[n, :, i*stride:i*stride+pool_height, j*stride:j*stride+pool_width]
                img_reshaped = img.reshape(C, -1) # Collapse the 2D spatial dimensions
                _, idx = torch.max(img_reshaped, dim=1) # Find the index of the maximum value

                localdx = torch.zeros_like(img_reshaped) # Create a tensor the same size as the image volume
                # Locally, dL/dx = dL/dy * dy/dx = dout * dy/dx
                # dy/dx = 1 at the maximum values and 0 elsewhere
                localdx[torch.arange(C), idx] = 1 * dout[n, :, i, j]
                localdx = localdx.reshape_as(img)
                dx[n, :, i*stride:i*stride+pool_height, j*stride:j*stride+pool_width] += localdx
...
class ThreeLayerConvNet(object):
    ...
    def __init__(self,
        ...
        ######################################################################
        # Replace "pass" statement with your code
        C, Hin, Win = input_dims

        # Convolution
        W1 = torch.randn(num_filters, C, filter_size, filter_size, dtype=dtype, device=device) * weight_scale
        b1 = torch.zeros(num_filters, dtype=dtype, device=device)

        # Hidden layer
        W2 = torch.randn(num_filters*(Hin//2)*(Win//2), hidden_dim, dtype=dtype, device=device) * weight_scale
        b2 = torch.zeros(hidden_dim, dtype=dtype, device=device)

        # Output layer
        W3 = torch.randn(hidden_dim, num_classes, dtype=dtype, device=device) * weight_scale
    ...
    def loss(self, X, y=None):
        ...
        ######################################################################
        # Replace "pass" statement with your code
        conv, cache1 = Conv_ReLU_Pool.forward(X, self.params["W1"], self.params["b1"], conv_param, pool_param)
        hidden, cache2 = Linear_ReLU.forward(conv, self.params["W2"], self.params["b2"]) # NOTE: Linear_ReLU flattens conv for us
        scores, cache3 = Linear.forward(hidden, self.params["W3"], self.params["b3"])
        ######################################################################
        #                             END OF YOUR CODE                       #
        ######################################################################
        ...
        loss += self.reg * torch.sum(self.params["W1"] * self.params["W1"]) # Add L2 regularisation
        loss += self.reg * torch.sum(self.params["W2"] * self.params["W2"])
        loss += self.reg * torch.sum(self.params["W3"] * self.params["W3"])

        # Backprop through layer three
        dx, dw, db = Linear.backward(dx, cache3)
        dw += 2*self.reg*self.params["W3"] # Add L2 regularisation
        grads.update({
            "W3": dw,
            "b3": db 
        })
        ...
        dw += 2*self.reg*self.params["W2"] # Add L2 regularisation
        grads.update({
            "W2": dw,
            "b2": db 
        })
        ...
        dw += 2*self.reg*self.params["W1"] # Add L2 regularisation
        grads.update({
            "W1": dw,
            "b1": db 
        })
...
class DeepConvNet(object):
    ...
    def __init__(self,
        ...
        # Track the size of the input as it moves through the network
        Hout, Wout = H, W

        # Initialise convolution weights
        for i in range(1, self.num_layers):
          if weight_scale == "kaiming":
            w = kaiming_initializer([C, *num_filters][i-1], num_filters[i-1], K=3, relu=True, device=device, dtype=dtype)
          else:
            w = torch.randn(num_filters[i-1], [C, *num_filters][i-1], 3, 3, dtype=dtype, device=device) * weight_scale
          b = torch.zeros(num_filters[i-1], dtype=dtype, device=device)
          ...
          if self.batchnorm:
            gamma = torch.ones(num_filters[i-1], dtype=dtype, device=device)
            beta = torch.zeros(num_filters[i-1], dtype=dtype, device=device)
            self.params.update({
                f"gamma{i}": gamma,
                f"beta{i}": beta,
            })
        ...
        if weight_scale == "kaiming":
          w = kaiming_initializer(num_filters[-1]*Hout*Wout, num_classes, relu=False, device=device, dtype=dtype)
        else:
          w = torch.randn(num_filters[-1]*Hout*Wout, num_classes, dtype=dtype, device=device) * weight_scale
        b = torch.zeros(num_classes, dtype=dtype, device=device)
        self.params.update({
            f"W{self.num_layers}": w,
            f"b{self.num_layers}": b,
        })
        ################################################################
        #                      END OF YOUR CODE                        #
        ################################################################
    ...
    def loss(self, X, y=None):
        ...
        scores, cache = Linear.forward(out, self.params[f"W{self.num_layers}"], self.params[f"b{self.num_layers}"]) # NOTE: Linear flattens the input for us
        caches.append(cache)
        #####################################################
        #                 END OF YOUR CODE                  #
        #####################################################
        ...
        loss += 0.5 * self.reg * torch.sum(self.params[f"W{self.num_layers}"] * self.params[f"W{self.num_layers}"]) # Add L2 regularisation
        dx, dw, db = Linear.backward(dx, caches.pop(-1))
        dw += self.reg * self.params[f"W{self.num_layers}"] # Add L2 regularisation
        grads.update({
            f"W{self.num_layers}": dw,
            f"b{self.num_layers}": db 
        })

        # Backprop through the remaining layers
        for i in range(self.num_layers-1, 0, -1):
          loss += 0.5 * self.reg * torch.sum(self.params[f"W{i}"] * self.params[f"W{i}"]) # Add L2 regularisation
          if (i-1) in self.max_pools:
              if self.batchnorm:
                  dx, dw, db, dgamma, dbeta = Conv_BatchNorm_ReLU_Pool.backward(dx, caches.pop(-1))
              else:
                  dx, dw, db = Conv_ReLU_Pool.backward(dx, caches.pop(-1))
          else:
              if self.batchnorm:
                  dx, dw, db, dgamma, dbeta = Conv_BatchNorm_ReLU.backward(dx, caches.pop(-1))
              else:
                  dx, dw, db = Conv_ReLU.backward(dx, caches.pop(-1))

          dw += self.reg*self.params[f"W{i}"] # Add L2 regularisation
          grads.update({
              f"W{i}": dw,
              f"b{i}": db 
          })
...
def create_convolutional_solver_instance(data_dict, dtype, device):
    ...
    # Get the input dimensions of the data
...
def kaiming_initializer(Din, Dout, K=None, relu=True, device='cpu',
    ...
    if K is None:
        ...
        # TODO Implement and check the above code
        ###################################################################
        #                         END OF YOUR CODE                        #
        ###################################################################
    return weight
...
class BatchNorm(object):
    ...
    def forward(x, gamma, beta, bn_param):
        ...
        if mode == 'train':
            ...
            var = torch.mean(x_centered**2, dim=0)
            ...
            cache = (x, x_norm, mean, var, eps, gamma)
            ################################################################
            #                           END OF YOUR CODE                   #
            ################################################################
        elif mode == 'test':
    ...
    def backward(dout, cache):
        ...
        x, x_norm, mean, var, eps, gamma = cache
        N, D = dout.shape

        # dL/dx_norm = dL/dy * dy/dx_norm
        # dx_norm = dout * dy/dx_norm
        # dx_norm = dout * d/dx_norm (gamma*x_norm + beta)
        # dx_norm = dout * gamma
        dx_norm = dout * gamma # (N, D) * (D,) = (N, D) [NOTE: (D,) is broadcasted to (N, D) for elementwise mult.]

        # dL/dvar = dL/dx_norm * dx_norm/dvar
        # dvar = dx_norm * dx_norm/dvar
        # dvar = dx_norm * d/dvar ((x_shifted) / torch.sqrt(var + eps))
        # dvar = dx_norm * ((x_shifted) * (-0.5) / (var + eps)**1.5)
        dvar = torch.sum(dx_norm * (-0.5) * (x - mean) * ((var + eps)**(-1.5)), dim=0)

        # dL/dmean = dL/dx_norm * dx_norm/dmean + dL/dvar * dvar/dmean
        # dmean = dx_norm * dx_norm/dmean + dvar * dvar/dmean
        # dmean = dx_norm * d/dmean ((x - mean) / torch.sqrt(var + eps)) + dvar * d/dmean (torch.mean(x_shifted**2))
        # dmean = dx_norm * (-1) / torch.sqrt(var + eps) + dvar * torch.mean(-2*x_shifted)
        dmean = torch.sum(dx_norm * (-1.0) / torch.sqrt(var + eps), dim=0) + dvar * torch.mean(-2.0*(x - mean), dim=0)

        # dL/dx = dL/dx_norm * dx_norm/dx + dL/dmean * dmean/dx + dL/dvar * dvar/dx
        # dx = dx_norm * dx_norm/dx + dmean * dmean/dx + dvar * dvar/dx
        # dx = dx_norm * d/dx ((x - mean) / torch.sqrt(var + eps)) + dmean * d/dx (torch.mean(x)) + dvar * d/dx (torch.mean((x-mean)**2))
        # dx = dx_norm * ((1) / torch.sqrt(var + eps)) + dmean * (1/m) + dvar * (2*torch.mean(x-mean))
        dx = dx_norm * (1.0/torch.sqrt(var + eps)) + (dmean/N) + (dvar * 2.0 * (x - mean) / N)

        # dL/dgamma = dL/dy * dy/dgamma
        # dgamma = dout * dy/dgamma
        # dgamma = dout * d/dgamma (gamma*x_norm + beta)
        # dgamma = dout * x_norm
        dgamma = torch.sum(dout*x_norm, dim=0) # (D, N) * (N, D)

        # dL/dbeta = dL/dy * dy/dbeta
        # dbeta = dout * dy/dbeta
        # dbeta = dout * d/dbeta (gamma*x_norm + beta)
        # dbeta = dout * 1
    ...
    def backward_alt(dout, cache):
        ...
        dx = (1.0/N) * (1.0/torch.sqrt(var + eps)) * (N*dx_norm - torch.sum(dx_norm, dim=0) - x_norm*torch.sum(dx_norm*x_norm, dim=0))
...
class SpatialBatchNorm(object):
    ...
    def forward(x, gamma, beta, bn_param):
        ...
        ## Spatial batch norm computes statistics over both the minibatch dimension N and the spatial dimensions H and W
        
        # Get input dimensions
        N, C, H, W = x.shape

        # First transpose from (N, C, H, W) to (C, N, H, W) so that the reshaping works correctly
        x_t = torch.transpose(x, 0, 1) # i.e. swaps dimensions 0 and 1

        # Collapse the spatial dimensions to give shape (N*H*W, C)
        x_reshaped = torch.reshape(x_t, (C, -1)).T

        # Pass through vanilla BatchNorm
        out, cache = BatchNorm.forward(x_reshaped, gamma, beta, bn_param)

        # Reshape back to correct shape
        out = out.T.reshape(C, N, H, W).transpose(0, 1)
        ################################################################
        #                       END OF YOUR CODE                       #
        ################################################################
    ...
    def backward(dout, cache):
        ...
        # First transpose from (N, C, H, W) to (C, N, H, W) so that the reshaping works correctly
        dout_t = torch.transpose(dout, 0, 1) # i.e. swaps dimensions 0 and 1

        # Collapse the spatial dimensions to give shape (N*H*W, C)
        dout_reshaped = torch.reshape(dout_t, (C, -1)).T

        # Pass backwards through vanilla BatchNorm 
        dx, dgamma, dbeta = BatchNorm.backward(dout_reshaped, cache)

        # Reshape dx back to correct shape
        dx = dx.T.reshape(C, N, H, W).transpose(0, 1)
        ##################################################################
        #                       END OF YOUR CODE                         #
        ##################################################################
