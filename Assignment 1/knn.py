...
def compute_distances_two_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    for train_idx in range(num_train):
      for test_idx in range(num_test):
        train = x_train[train_idx].flatten()
        test = x_test[test_idx].flatten()
        dists[train_idx, test_idx] = torch.sum(torch.pow(train-test, 2))
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def compute_distances_one_loop(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    for train_idx in range(num_train):
      train = x_train[train_idx].flatten()
      dists[train_idx, :] = torch.sum(torch.pow(x_test_flattened-train, 2), dim=1)
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def compute_distances_no_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    # Calculate Euclidean distance as (X-Y)^2 = ||X||^2 + ||Y||^2 - 2XY^T
    # where ||X||^2 is the sum of the squares in X
    xsqrd = torch.sum(torch.pow(x_train_flattened, 2), dim=1).view(-1, 1)
    ysqrd = torch.sum(torch.pow(x_test_flattened, 2), dim=1).view(1, -1)
    twoxy = 2 * torch.matmul(x_train_flattened, x_test_flattened.t())
    dists = xsqrd + ysqrd - twoxy
...
def predict_labels(dists: torch.Tensor, y_train: torch.Tensor, k: int = 1):
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    _, topk_indices = torch.topk(dists, k=k, dim=0, largest=False, sorted=True)
    y_pred, _ = torch.mode(y_train[topk_indices], dim=0)
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return y_pred
...
def knn_cross_validate(
    ...
    for k in k_choices:
      ...
      for i in range(num_folds):
        ...
        # Evaluate the classifier using this fold and this value of k
        classifier = KnnClassifier(x_train_fold, y_train_fold)
        accuracy = classifier.check_accuracy(x_train_folds[i], y_train_folds[i], k=k, quiet=True)
...
def knn_get_best_k(k_to_accuracies: Dict[int, List]):
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    best_mean_accuracy = -1 # Initialise best mean accuracy as negative
