...
def count_tensor_elements(x: Tensor) -> int:
    ...
    # Replace "pass" statement with your code
    num_elements = 1
    for dim in x.shape:  # Iterate through each dimension
      num_elements *= dim  # Multiply by the size of each dimension
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return num_elements
...
def multiples_of_ten(start: int, stop: int) -> Tensor:
    ...
    # Find the first multiple of 10 including or after "start"
    first_multiple = (start + 9) // 10 * 10

    # Return an empty tensor if there are no multiples of ten within the range
...
def take_one_elem_per_col(x: Tensor) -> Tensor:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    idx1 = torch.tensor([1, 0, 3]) # Row indices
    idx2 = torch.tensor([0, 1, 2]) # Column indices
    y = x[idx1, idx2]
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
...
def zero_row_min(x: Tensor) -> Tensor:
    ...
    # Replace "pass" statement with your code
    y = x.clone()
    idx1 = torch.arange(y.shape[0]) # Row indices
    idx2 = y.argmin(dim=1) # Column indices
    y[idx1, idx2] = 0
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
...
def normalize_columns(x: Tensor) -> Tensor:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    y = x.clone() # Make copy of x

    # Calculate means and standard deviations
    M = y.shape[0]
    means = y.sum(dim=0) / M
    stds = torch.sqrt((torch.pow(y - means, 2)).sum(dim=0) / (M - 1))
...
def challenge_mean_tensors(xs: List[Tensor], ls: Tensor) -> Tensor:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    # Concatenate list of 1D tensors
    x_cat = torch.cat(xs)
    
    # Calculate where the start of each tensor occurs in x_cat
    boundaries = torch.cat([torch.tensor([0], device=ls.device), ls.cumsum(dim=0)])[:-1]
    # print(f"{boundaries = }")

    # Create an indices vector that indicates which tensor each element in x_cat belongs to
    indices = torch.zeros(len(x_cat), device=ls.device, dtype=ls.dtype)
    indices[boundaries] = torch.ones(len(ls), device=indices.device, dtype=indices.dtype)
    indices = indices.cumsum(dim=0) - 1
    # print(f"{indices = }")

    # Create a matrix where each row corresponds to an original tensor from x
    expanded = torch.zeros((len(ls), len(x_cat)), device=x_cat.device, dtype=x_cat.dtype)
    expanded[indices, torch.arange(len(x_cat), device=indices.device, dtype=indices.dtype)] = x_cat
    # print(f"{expanded = }")

    # Sum each row and divide by their corresponding length
    y = expanded.sum(dim=1) / ls
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return y
...
def challenge_get_uniques(x: torch.Tensor) -> Tuple[Tensor, Tensor]:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    # Get sorted values and indices
    sorted_x, sort_indices = torch.sort(x)
    # print(f"{sorted_x, sort_indices = }")
    
    # Find where the values change in the sorted tensor
    changes = torch.ones_like(sorted_x, dtype=torch.bool)
    changes[1:] = sorted_x[1:] != sorted_x[:-1]
    # print(f"{changes = }")

    # Get the unique values at change positions
    uniques = sorted_x[changes]
    # print(f"{uniques = }")

    # Use argmax to find where x is first equal to the unique values
    indices = (x == uniques.view(-1,1)).to(torch.int8).argmax(dim=1)
    # print(f"{indices = }")
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
