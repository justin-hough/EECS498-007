...
def generate_token_dict(vocab):
    ...
    token_dict = {
        v: idx for idx, v in enumerate(vocab)
    }
...
def prepocess_input_sequence(
    ...
    # Each word in the input is separated by a space
    for word in input_str.split(" "):
        # Special tokens can be handled easily
        if word in spc_tokens:
            out.append(token_dict[word])
        else: # There are multiple numbers per word
            for number in word:
                out.append(token_dict[number])
...
def scaled_dot_product_two_loop_single(
    ...
    # Compute the query-key similarity using two for loops
    similarity = torch.zeros(K, K)
    for i, q in enumerate(query):
        for j, k in enumerate(key):
            similarity[i, j] = torch.dot(q, k)/(M**0.5)

    # Softmax the similarity to get weights
    weights = F.softmax(similarity, dim=1) # (K, K)

    # Compute the output as the weighted sum of values
    out = weights @ value # (K, M) = (K, K) @ (K, M)
...
def scaled_dot_product_two_loop_batch(
    ...
    ## Copied my scaled_dot_product_two_loop_single implementation and edited for batches

    # Compute the query-key similarity using two for loops
    similarity = torch.zeros(N, K, K)
    for i in range(K):
        for j in range(K):
            similarity[:, i:i+1, j:j+1] = torch.bmm(
                query[:, i, :].view(N, 1, M),
                key[:, j, :].view(N, M, 1)
                )/(M**0.5)

    # Softmax the similarity to get weights
    weights = F.softmax(similarity, dim=2) # (N, K, K)

    # Compute the output as the weighted sum of values
    out = weights @ value # (N, K, M) = (N, K, K) @ (N, K, M)
...
def scaled_dot_product_no_loop_batch(
    ...
    # Compute the query-key similarity using matrix-matrix multiplication
    similarity = (query @ key.permute(0, 2, 1))/(M**0.5) # (N, K, K) = (N, K, M) @ (N, M, K)
    ...
    if mask is not None:
        ...
        # Applying mask to the similarity matrix (BEFORE softmax is performed)
        similarity[mask] = -1e9

    # Replace "pass" statement with your code

    # Softmax the similarity to get weights
    weights_softmax = F.softmax(similarity, dim=2) # (N, K, K)

    # Compute the output as the weighted sum of values
    y = weights_softmax @ value # (N, K, M) = (N, K, K) @ (N, K, M)
...
class SelfAttention(nn.Module):
    def __init__(self, dim_in: int, dim_q: int, dim_v: int):
        ...
        # Initialise linear layers to make key, query, and value vectors
        ...
        # Initialise the weights as instructed
...
class MultiHeadAttention(nn.Module):
    def __init__(self, num_heads: int, dim_in: int, dim_out: int):
        ...
        # Step 1. Initialise list of SingleHeadAttention layers
        self.heads = nn.ModuleList([SelfAttention(dim_in=dim_in, dim_q=dim_out, dim_v=dim_out) for _ in range(num_heads)])

        # Step 2. Initialise linear layer to map output of self.heads back to dim_in
        self.linear = nn.Linear(in_features=num_heads*dim_out, out_features=dim_in)
    ...
    def forward(
        ...
        y_heads = [head(query, key, value, mask) for head in self.heads]

        # Concate the outputs
        y_heads = torch.cat(y_heads, dim=2)

        # Map the concatenated outputs from each head back to the desired output dimension
        y = self.linear(y_heads)
...
class LayerNormalization(nn.Module):
    def __init__(self, emb_dim: int, epsilon: float = 1e-10):
        ...
        # Initialise the scale and shift parameters as 1 and 0 respectively (i.e. no scale, no shift)
    ...
    def forward(self, x: Tensor):
        ...
        mean = x.mean(dim=-1).unsqueeze(dim=-1)

        # Calculate the standard deviation along the embedding dimension
        std = ((torch.mean((x - mean)**2, dim=-1))**0.5).unsqueeze(dim=-1)
        # NOTE: Using torch.std() with correction=0 also works... but instructed not to
...
class FeedForwardBlock(nn.Module):
    def __init__(self, inp_dim: int, hidden_dim_feedforward: int):
        ...
        # The shape of the input and output from all Transformer blocks is the same
        ...
        # Initialise the weights as instructed in SelfAttention
...
class EncoderBlock(nn.Module):
    def __init__(
       ...
       inp - multi_head_attention - out1 - layer_norm(out1 + inp) - dropout - out2 \ 
        ...
        # Step 1. Initialise MultiHeadAttention block
        self.multiHeadAttention = MultiHeadAttention(num_heads=num_heads, dim_in=emb_dim, dim_out=emb_dim//num_heads)

        # Step 2. Initialise the two LayerNorm layers
        self.layerNorm1 = LayerNormalization(emb_dim=emb_dim)
        self.layerNorm2 = LayerNormalization(emb_dim=emb_dim)

        # Step 3. Initialise the FeedForward block
        self.feedForward = FeedForwardBlock(inp_dim=emb_dim, hidden_dim_feedforward=feedforward_dim)

        # Step 4. Initialise the dropout layer
    ...
    def forward(self, x):
        ...
        out1 = self.multiHeadAttention(query=x, key=x, value=x)

        # Step 2. Residual connection, LayerNorm, dropout
        out2 = self.dropout(self.layerNorm1(x + out1))

        # Step 3. Feedforward
        out3 = self.feedForward(out2)

        # Step 4. Residual connection, LayerNorm, dropout
...
def get_subsequent_mask(seq):
    ...
    # The first row should only see the first column (i.e. False, then all True)
    # The second row should see the first two columns (i.e. False, False, then all True)
    # Therefore, we want an upper triangular matrix of Trues (excluding the main diagonal)
    mask = torch.triu(seq.new_full((N, K, K), fill_value=True, dtype=torch.bool), diagonal=1)
...
class DecoderBlock(nn.Module):
    def __init__(
        ...
        # Step 1. Initialise two MultiHeadAttention block
    ...
    def forward(
        ...
        # Step 3. Cross attention between the inputs and targets
...
def position_encoding_simple(K: int, M: int) -> Tensor:
    ...
    # Use arange to create the 1D tensor, then make the right shape
    y = torch.arange(start=0, end=K)/K
    y = y.view(-1, 1).repeat(1, M).view(1, K, M)
...
def position_encoding_sinusoid(K: int, M: int) -> Tensor:
    ...
    # Fill y with p/10000^a, where a = floor(2i/M)
    # p = row index and i = column index // 2
    import math
    i = torch.arange(start=0, end=math.ceil(M/2)).repeat_interleave(2)
    a = torch.floor((2*i)/M)
    y = torch.arange(start=0, end=K).view(-1, 1) * (1/(10000**a)).view(-1, 1).T

    # Take the sin of even columns and cos of odd columns
    y[:, ::2] = torch.sin(y[:, ::2])
...
class Transformer(nn.Module):
    ...
    def forward(
        ...
        # Step 1. Pass the question input embeddings through the encoder
        enc_out = self.encoder(q_emb_inp)

        # Step 2. Construct an appropriate mask.
        # NOTE: The decoder doesn't need the last element in the answer
        mask = get_subsequent_mask(ans_b[:, :-1])

        # Step 3. Pass the answer input embeddings, encoder output, and mask to the decoder
        dec_out = self.decoder.forward(target_seq=a_emb_inp, enc_out=enc_out, mask=mask)
        # Reshape to (N*O, M) as requested
