...
def nn_forward_pass(params: Dict[str, torch.Tensor], X: torch.Tensor):
    ...
    x = torch.matmul(X, W1).add(b1) # FC layer #1
    hidden = torch.clamp(x, min=0) # ReLU
    scores = torch.matmul(hidden, W2).add(b2) # FC layer #2
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def nn_forward_backward(
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    dscores = (torch.exp(scores)/torch.sum(torch.exp(scores), dim=1).view(-1,1))
    ...
    # The bias term is an add operation (i.e. a gradient distributor), so db2 == dscores.
    # Sum and divide by N to get average across the N samples
    ...
    # The bias term is an add operation (i.e. a gradient distributor), so db1 == dh1.
    # Sum and divide by N to get average across the N samples
...
def nn_get_search_params():
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    learning_rates = torch.logspace(-1, 1, steps=5)
    hidden_sizes = [500]
    regularization_strengths = torch.cat((torch.tensor([0]), torch.logspace(-10, -2, steps=5)))
    learning_rate_decays = torch.linspace(0.75, 1, steps=5)
    ###########################################################################
    #                           END OF YOUR CODE                              #
    ###########################################################################
...
def find_best_net(
    ...
    print(f"{learning_rates, hidden_sizes, regularization_strengths, learning_rate_decays = }")
        ...
        for hs in hidden_sizes:
            for reg in regularization_strengths:
                for lrd in learning_rate_decays:
                    i += 1
                    print('Training NN %d / %d with learning_rate=%e, hidden_size=%d, reg=%e, lr_decay=%f'
                        % (i, num_models, lr, hs, reg, lrd))
                    ...
                    if stats["val_acc_history"][-1] > best_val_acc:
                        ...
                        best_stat = stats
                        best_net = net
                        print(f"New best validation accuracy of: {best_val_acc}")
